    ...
    <v-container>
        <v-col cols="12">
            ...
            <div class="text-center mt-1 mb-6" style="font-size: 18px; font-weight: 500; color: #1976D2;">
                แก้ไขสวัสดิการสำหรับตัวเองเพื่อสุขภาพ
            </div>
            <v-form>
                <v-col>
                    <div>
...
export default {
    ...
    methods: {
        ...
        async update() {
            await this.$fire.firestore.collection("members").doc(this.$store.getters.getLine.userId).collection("info").doc("benefit").update(this.form).then((res) => {
