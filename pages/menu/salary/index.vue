                ...
                <v-col>
                    <div>
                        ...
                        <v-select label="เลือกอาชีพของท่าน" :items="job_value" v-model="form.job"
                            :menu-props="{ offsetY: true }" readonly></v-select>
                    </div>
                    <div class="mt-8">
                        <p style="font-size: 16px; font-weight: 500;">ระบุรายได้ต่อปีของท่าน</p>
                        <v-text-field v-model="form.salary" name="salary" label="จำนวนเงิน" id="id" readonly></v-text-field>
                    </div>
                </v-col>
                <div class="text-center mt-7">
                    <v-btn color="primary" class="next-btn" width="283" @click="editSalary">แก้ไขข้อมูล</v-btn>
                </div>
            </v-form>
        </v-col>
    </v-container>
...
export default {
    ...
    methods: {
        ...
        editSalary() {
            this.$router.push('/menu/salary/editSalary')
        }
    }
};
</script>