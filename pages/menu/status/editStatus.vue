            ...
            <v-form>
                <v-col>
                    <v-select label="เลือกสถานภาพของท่าน" :items="['โสด', 'สมรส', 'หย่าร้าง', 'ม่าย', 'แยกกันอยู่']"
                        v-model="form.status" :menu-props="{ offsetY: true }"></v-select>
    ...
    methods: {
        ...
        async update() {
            if (this.validate()) {
                ...
                await fetch(`http://localhost:8080/status/${this.form.user_id}`, requestOptions)
                    .then(response => response.json())
                    .then(result => {
